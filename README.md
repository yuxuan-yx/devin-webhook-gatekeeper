...
A Dockerized control plane that turns engineering events — from any source —
into autonomous Devin sessions, and produces a pull request, an audit trail,
and a metric.
...
Set `DEVIN_API_KEY` (a service-user key, prefix `cog_`) and `DEVIN_ORG_ID`
(prefix `org-`, from Settings → Devin API); `PLAYBOOK_ID_*` and
`KNOWLEDGE_ID_REPO_CONTEXT` are optional.

Point a GitHub webhook at `https://<host>/webhook/github`, content type
`application/json`, same secret; point scanners at `https://<host>/events/scan`.
For local testing, expose it with `ngrok http 8000` or
`cloudflared tunnel --url http://localhost:8000`.

## Simulate the workflow (no GitHub, no Devin account)

```bash
# 1. signed GitHub + scanner events, a forged signature, then /stats
python simulate.py --secret "$GITHUB_WEBHOOK_SECRET"

# 2. the policy layer, in isolation
...
- **Structured JSON logs** on every path (`signature_invalid`, `event_dropped`,
  ...
  carrying a correlation id — `X-GitHub-Delivery` for webhooks, `X-Delivery-Id`
  for scanners — and the Devin `session_id` on success.
...
- **Fleet report** (`python report.py`, runnable on any schedule):
